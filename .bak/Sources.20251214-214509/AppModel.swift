    ...
    private func runReboot(debugMode: Bool) async {
        do {
            ...
            let base = URL(string: "http://\(host)/")!
                DispatchQueue.main.async { self.progress = 0.10; self.touch() }
            ...
            try Task.checkCancellation()
                DispatchQueue.main.async { self.progress = 0.20; self.touch() }
            ...
            try Task.checkCancellation()
                DispatchQueue.main.async { self.progress = 0.30; self.touch() }
            ...
            for i in 1...240 {
                ...
                if i % 2 == 0 {
                    ...
                    if !wanDownSeen && wFail >= wanDownThreshold {
                        wanDownSeen = true
                DispatchQueue.main.async { self.internet = .offline; self.touch() }
                        log.write("WAN_DOWN at i=\(i)")
                    }
                    if wanDownSeen && !wanUpSeen && wOk >= wanUpThreshold {
                        wanUpSeen = true
                        observedWanUpAt = Date()
                DispatchQueue.main.async { self.internet = .online; self.touch() }
