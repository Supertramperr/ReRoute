...
case idle
    ...
    private func runReboot(debugMode: Bool) async {
        do {
            ...
            let graceSeconds = 2
            ui { self.operation = .starting; self.progress = 0.0; self.startingCountdown = graceSeconds; self.touch() }

            let graceSeconds = 2
            ui { self.startingCountdown = graceSeconds; self.progress = 0.0; self.touch() }
            if graceSeconds > 0 {
